...
def calculate_returns_for_large_dataset(signals_file, data_file, output_file):
    ...
    print("="*80)
    print("LARGE-SCALE RETURN CALCULATION - ORIGINAL POSITION RULES")
    print("="*80)
    print("Position Management Rules (ORIGINAL):")
    print("  - Entry: Position opens at timepoint AFTER signal calculation")
    print("  - Exit: Close after 5 positive returns OR at 20 timepoints maximum")
    print("  - Long positions: Profit when price goes up")
    ...
    # Load price data optimized for large datasets
    print("Loading price data (this may take a while for large datasets)...")
    ...
    if all_results:
        ...
        # Closure analysis
        early_close = results_df[results_df['Position_Closed'] == True]
        late_close = results_df[results_df['Position_Closed'] == False]
        
        if len(early_close) > 0:
            print(f"\nEarly closure (5+ positive returns):")
            print(f"  Count: {len(early_close)}")
            print(f"  Mean return: {early_close['Final_Return'].mean():.4f}")
            print(f"  Average holding period: {early_close['Holding_Period'].mean():.1f} days")
        
        if len(late_close) > 0:
            print(f"\nHeld to limit (20 days):")
            print(f"  Count: {len(late_close)}")
            print(f"  Mean return: {late_close['Final_Return'].mean():.4f}")
...
def main():
    ...
    # File paths
    signals_file = os.path.join(RESULTS_DIR, 'trading_signals_150_timepoints_original_criteria.csv')
    data_file = 'data.csv'  # Original data file
    output_file = os.path.join(RETURNS_DIR, 'trading_signals_with_returns_150_timepoints_original_criteria.csv')
    ...
    if not os.path.exists(signals_file):
        ...
        print("Please wait for the large-scale analysis to complete first.")
        return
    ...
    if not results_df.empty:
        ...
        print(f"  Date range: Based on timepoints {results_df['Start Timepoint'].min()}-{results_df['End Timepoint'].max()}")
