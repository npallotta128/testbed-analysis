...
USE_CACHE = True
NUM_PROCESSES = min(8, mp.cpu_count())  # Increased for large dataset
CHUNK_SIZE = 25  # Process in chunks to manage memory
...
def preprocess_data_cached(data, start_timepoint, end_timepoint):
    """Preprocess data with caching and memory optimization"""
    ...
    log_memory_usage(f"before preprocessing {start_timepoint}")
    
    # Create a subset of data to reduce memory usage
    data_subset = data.iloc[start_timepoint*100:(end_timepoint+10)*100]  # Rough estimate of rows needed
    
    # Pivot the data to create tables
    closing_table = data_subset.pivot_table(
        index='Date', columns='Symbol', values='Closing', aggfunc='first'
    ).iloc[start_timepoint:end_timepoint]
    
    volume_table = data_subset.pivot_table(
        index='Date', columns='Symbol', values='Volume', aggfunc='first'
    ).iloc[end_timepoint-10:end_timepoint]
    ...
    # Clean up
    del data_subset, closing_table, volume_table
    gc.collect()
    
    # Save to cache
    save_to_cache(result, cache_file)
    log_memory_usage(f"after preprocessing {start_timepoint}")
    return result

def normalize_data_cached(closing_table, variable_names, timepoint):
    """Normalize data with caching and memory optimization"""
    ...
    log_memory_usage(f"before normalization {timepoint}")
    ...
    save_to_cache(result, cache_file)
    log_memory_usage(f"after normalization {timepoint}")
    return result

def perform_clustering_cached(normalized_data, timepoint):
    """Perform clustering with caching and memory optimization"""
    ...
    log_memory_usage(f"before clustering {timepoint}")
    
    # Use scipy's hierarchical clustering with ward linkage
    clustering_data = normalized_data[:, :1501]
    
    # Perform hierarchical clustering with ward linkage
    linkage_matrix = linkage(clustering_data, method='ward')
    clusters = fcluster(linkage_matrix, 50, criterion='distance') - 1
    ...
    log_memory_usage(f"after clustering {timepoint}")
    return clusters

def process_single_timepoint(args):
    """Process a single timepoint with memory optimization"""
    timepoint_idx, start_timepoint, data_file = args
    
    try:
        # Load data in chunks if needed
        print(f"Loading data for timepoint {timepoint_idx + 1}...")
        data = pd.read_csv(data_file)
        
        current_start_timepoint = start_timepoint + timepoint_idx
        end_timepoint = current_start_timepoint + 2001
        
        print(f"Processing timepoint {timepoint_idx + 1}: Start {current_start_timepoint}, End {end_timepoint}")
        log_memory_usage(f"start timepoint {timepoint_idx + 1}")
        ...
        log_memory_usage(f"end timepoint {timepoint_idx + 1}")
...
def main():
    """Main function for large-scale analysis"""
    print("="*80)
    print("LARGE-SCALE FINANCIAL ANALYSIS - 150 TIMEPOINTS")
    print("="*80)
    ...
    for chunk_id, chunk_timepoints in enumerate(chunks):
        ...
        print(f"Estimated remaining: {(elapsed_total / (chunk_id + 1)) * (len(chunks) - chunk_id - 1):.2f}s")
    ...
    if all_results:
        ...
        final_filename = os.path.join(RESULTS_DIR, 'all_trading_signals_150_timepoints.csv')
    ...
    if not final_results.empty:
        ...
        # Sample results
        print("\nFirst 10 results:")
        print(final_results.head(10)[['Variable Name', 'Type', 'State', 'Price', 'Timepoint Index', 'End Timepoint']])
    else:
