...
def load_price_data_optimized(data_file):
    """Load price data with memory optimization"""
    print("Loading price data with optimization...")
    log_memory_usage("before loading price data")
    
    # Read in chunks and create pivot table efficiently
    chunk_size = 100000
    chunks = []
    ...
    print(f"Loaded {len(chunks)} chunks of data")
...
def calculate_position_return_optimized(price_data, symbol, open_timepoint, signal_type, entry_price):
    """
    Optimized return calculation for a single position
    """
    max_hold_period = 20
    positive_returns_needed = 5
    
    # Get symbol column index for faster lookup
...
def process_signal_chunk(args):
    ...
    print(f"Processing return chunk {chunk_id} with {len(chunk_signals)} signals...")
    log_memory_usage(f"start chunk {chunk_id}")
    ...
    for idx, row in chunk_signals.iterrows():
        ...
        # Position opens at timepoint after signal calculation
        ...
        # Calculate returns
        returns_data = calculate_position_return_optimized(
    ...
    log_memory_usage(f"end chunk {chunk_id}")
    print(f"Processed chunk {chunk_id}: {len(results)} valid returns")
    
    return results

def calculate_returns_large_scale(signals_file, data_file, output_file):
    """Calculate returns for large-scale signals dataset"""
    
    print("="*80)
    print("LARGE-SCALE RETURN CALCULATION")
    print("="*80)
    ...
    # Load price data with optimization
    price_pivot, dates = load_price_data_optimized(data_file)
    ...
    print(f"Split signals into {len(signal_chunks)} chunks of max {CHUNK_SIZE} signals each")
    ...
    with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
        ...
        for future in as_completed(future_to_chunk):
            ...
            try:
                ...
                print(f"Completed chunk {chunk_id} in {elapsed:.2f}s total")
...
def main():
    ...
    # File paths
    signals_file = os.path.join(RESULTS_DIR, 'all_trading_signals_150_timepoints.csv')
    data_file = 'data.csv'  # Original data file
    output_file = os.path.join(RETURNS_DIR, 'trading_signals_with_returns_150_timepoints.csv')
    ...
    if not os.path.exists(signals_file):
        ...
        print("Please run the large-scale analysis first.")
        return
    
    log_memory_usage("startup")
    
    start_time = time.time()
    results_df = calculate_returns_large_scale(signals_file, data_file, output_file)
    total_duration = time.time() - start_time
    
    print("\n" + "="*80)
    print("LARGE-SCALE RETURN CALCULATION COMPLETE!")
    print("="*80)
    print(f"Total computation time: {total_duration:.2f} seconds ({total_duration/3600:.2f} hours)")
    
    if not results_df.empty:
        print(f"Results ready for Monte Carlo simulations:")
