"""
Capital Allocation - No Staging (Instant Entry/Exit)

Simplified model:
- Deploy full position immediately (no 10-day entry window)
- Hold for 500 days (~2 years)
- Exit full position immediately at end of holding period
- 5% capital constraint per position
- 5% ADV volume constraint (default, configurable via env)
- 5% total per-security exposure cap
"""
...
VOLUME_CONSTRAINT_PCT = float(os.environ.get('VOLUME_CONSTRAINT_PCT', '0.05'))  # default 5% ADV
...
class SimplePortfolioSimulator:
    """Portfolio simulator with instant entry/exit, 2-year holds"""
    
    def __init__(self, initial_capital, max_position_pct=0.05, volume_pct=0.05):
    ...
    def get_exposure_to_security(self, security):
        """Total current exposure to a security"""
        return sum(pos['current_value'] for pos in self.positions if pos['security'] == security)
    
    def open_position(self, event_data, day):
        """Open position immediately (no staging)"""
        current_equity = self.get_current_equity()
        
        # Check per-security 5% exposure limit
        ...
        # Constrain by per-security limit
    ...
    def process_day(self, day):
        """Process positions: update values and close if holding period complete"""
        for pos in self.positions[:]:
            days_held = day - pos['entry_day']
            
            if days_held >= HOLDING_PERIOD_DAYS:
                # Holding period complete - exit immediately
                total_gain = pos['position_size'] * (pos['expected_return_pct'] / 100.0)
                final_value = pos['position_size'] + total_gain
                
                # Return capital
                self.cash += final_value
                
                # Record closed position
                pnl = final_value - pos['position_size']
                closed_pos = {
                ...
                expected_gain = pos['position_size'] * (pos['expected_return_pct'] / 100.0) * progress
                pos['current_value'] = pos['position_size'] + expected_gain
    
    def run_simulation(self, ranked_events):
        """Run simulation with instant entry/exit"""
        ...
        for idx, (_, event) in enumerate(ranked_events.iterrows()):
            day = idx
            ...
            if (idx + 1) % 1000 == 0:
                print(f"  Processed {idx+1}/{len(ranked_events)} events, "
        ...
        max_days = max([p['exit_day'] for p in self.positions]) if self.positions else day
        
        for d in range(day + 1, max_days + 1):
            self.process_day(d)
            if d % 100 == 0 and len(self.positions) > 0:
                print(f"  Day {d}: {len(self.positions)} positions still active")
        
        print(f"\nSimulation complete:")
        print(f"  Attempted: {len(ranked_events)}")
...
def run_no_staging_allocation(top_n=None):
    """Run capital allocation without staging"""
    ...
    if top_n:
        ...
        print(f"\nUsing top {top_n} events by model probability")
    ...
    print("\n" + "="*80)
    print(f"NO-STAGING CAPITAL ALLOCATION - 5% ADV, 2-YEAR HOLD")
    print("="*80)
    
    for idx, capital in enumerate(capital_levels, 1):
        print(f"\n[{idx}/{len(capital_levels)}] Testing ${capital/1e6:.0f}M capital...")
        
        simulator = SimplePortfolioSimulator(capital, MAX_POSITION_PCT, VOLUME_CONSTRAINT_PCT)
        ...
        if result:
            ...
            print(f"  Security Limit: {metrics['security_constrained_pct']:.1f}%")
            print(f"  Win Rate: {metrics['win_rate_pct']:.1f}%")
            
            if capital == capital_levels[-1]:
                positions_df.to_csv('no_staging_positions_1B.csv', index=False)
    
    if results:
        results_df = pd.DataFrame(results)
        results_df.to_csv('no_staging_allocation_results.csv', index=False)
        print(f"\n✓ Saved results to no_staging_allocation_results.csv")
...
if __name__ == '__main__':
    ...
    results = run_no_staging_allocation(top_n=top_n)
